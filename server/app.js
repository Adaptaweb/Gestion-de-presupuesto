    ...
    <body style="margin:0;padding:0;background:#f8fffc;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif">
      <table width="100%" cellpadding="0" cellspacing="0"><tr><td align="center" style="padding:40px 16px">
        <table width="480" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;padding:40px 32px;box-shadow:0 4px 20px rgba(0,0,0,0.05)">
          ...
          <tr><td align="center" style="padding-bottom:8px"><h1 style="margin:0;font-size:22px;font-weight:900;color:#1e293b">Casi listo</h1></td></tr>
          <tr><td align="center" style="padding-bottom:24px"><p style="margin:0;font-size:15px;color:#64748b;line-height:1.5">Gracias por registrarte en <strong>Kuentas Klaras</strong>. Para empezar, confirma tu correo electrónico.</p></td></tr>
          <tr><td align="center" style="padding-bottom:24px">
            <a href="${verifyUrl}" style="display:inline-block;background:#059669;color:#ffffff;padding:14px 36px;border-radius:12px;text-decoration:none;font-weight:800;font-size:15px">CONFIRMAR CUENTA</a>
          </td></tr>
          <tr><td align="center" style="padding-bottom:16px"><p style="margin:0;font-size:13px;color:#94a3b8">O copia este link en tu navegador:</p></td></tr>
          <tr><td align="center" style="padding-bottom:24px"><p style="margin:0;font-size:12px;color:#64748b;word-break:break-all;font-family:monospace">${verifyUrl}</p></td></tr>
          <tr><td align="center"><p style="margin:0;font-size:13px;color:#94a3b8">Si no creaste esta cuenta, ignora este mensaje.</p></td></tr>
...
function buildWelcomeEmail(nombre, loginUrl) {
  return `
    <!DOCTYPE html>
    <html><head><meta charset="utf-8"></head>
    <body style="margin:0;padding:0;background:#f8fffc;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif">
...
app.get('/api/auth/config', (req, res) => {
  try {
    res.json({ googleClientId: process.env.GMAIL_CLIENT_ID || '' });
  } catch (err) {
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    if (!PASSWORD_REGEX.test(password)) {
      return res.status(400).json({ error: 'La contraseña debe tener al menos 8 caracteres, mayúscula, minúscula, número y un caracter especial' });
    }
    ...
    await db.run(
      'INSERT INTO users (id, name, nombre, apellido, email, password, role, casilla, email_verified, verification_token) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      id, name, nombre, apellido, email, hashedPassword, role, casilla, false, verificationToken
    );
    ...
    if (resend) {
      try {
        await resend.emails.send({
          ...
          to: email,
          subject: 'Confirma tu cuenta en Kuentas Klaras',
          html: buildVerificationEmail(verifyUrl),
        });
      } catch (emailErr) {
        console.error('[Register] Error sending verification email:', emailErr.message);
      }
    }

    res.json({ success: true, message: 'Revisa tu correo para verificar tu cuenta' });
  } catch (error) {
    console.error(error);
    ...
    if (resend) {
      try {
        await resend.emails.send({
          from: 'Kuentas Klaras <noreply@adaptaweb.cl>',
          to: user.email,
          subject: '¡Bienvenido a Kuentas Klaras!',
          html: buildWelcomeEmail(user.nombre || user.name, loginUrl),
        });
      } catch (emailErr) {
        console.error('[VerifyEmail] Error sending welcome email:', emailErr.message);
      }
    }
...
app.post('/api/auth/resend-verification', async (req, res) => {
  try {
    ...
    if (!user) return res.json({ success: true, message: 'Si el email existe, recibirás un link de verificación' });
    if (user.email_verified) return res.json({ success: true, message: 'La cuenta ya está verificada' });
    ...
    if (resend) {
      try {
        ...
        console.error('[ResendVerification] Error:', emailErr.message);
      }
    }
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id: user.id, email: user.email, name: user.name, nombre: user.nombre, apellido: user.apellido, role: user.role }, JWT_SECRET, { expiresIn: '7d' });
    res.json({ token, user: { id: user.id, name: user.name, nombre: user.nombre, apellido: user.apellido, email: user.email, role: user.role, casilla: user.casilla } });
  } catch (error) {
    console.error(error);
...
app.post('/api/auth/google', async (req, res) => {
  try {
    ...
    let user = await db.get('SELECT * FROM users WHERE google_id = ? OR email = ?', sub, email);
    ...
    if (user) {
      ...
      await db.run(
        'INSERT INTO users (id, name, nombre, apellido, email, password, role, casilla, google_id, email_verified) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, TRUE)',
        id, name, nombre, apellido, email, hashedPassword, role, casilla, sub
      );
      ...
      if (resend) {
        try {
          ...
          await resend.emails.send({
            ...
            to: email,
            subject: '¡Bienvenido a Kuentas Klaras!',
            html: buildWelcomeEmail(nombre, loginUrl),
          });
        } catch (emailErr) {
          console.error('[Google] Error sending welcome email:', emailErr.message);
        }
      }
    }
...
app.get('/api/auth/verify', authenticateToken, async (req, res) => {
  const user = await db.get('SELECT id, name, nombre, apellido, email, role, casilla FROM users WHERE id = ?', req.user.id);
...
app.get('/api/admin/users', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const users = await db.all('SELECT id, name, nombre, apellido, email, role, blocked, created_at FROM users WHERE deleted_at IS NULL ORDER BY created_at DESC');
    res.json({ users });
  } catch (error) {
...
app.post('/api/admin/users', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const { nombre, apellido, name, email, password, role } = req.body;
    const finalNombre = nombre || '';
    const finalApellido = apellido || '';
    const finalName = name || `${finalNombre} ${finalApellido}`.trim();
    if (!finalName || !email || !password) return res.status(400).json({ error: 'Nombre, email y contraseña son requeridos' });
    if (!PASSWORD_REGEX.test(password)) {
      return res.status(400).json({ error: 'La contraseña debe tener al menos 8 caracteres, mayúscula, minúscula, número y un caracter especial' });
    }
    ...
    await db.run('INSERT INTO users (id, name, nombre, apellido, email, password, role, casilla, email_verified) VALUES (?, ?, ?, ?, ?, ?, ?, ?, TRUE)',
      id, finalName, finalNombre, finalApellido, email, hashedPassword, userRole, casilla);
    ...
    if (resend) {
      try {
        await resend.emails.send({
          ...
          to: email,
          subject: '¡Bienvenido a Kuentas Klaras!',
          html: buildWelcomeEmail(finalNombre || finalName, loginUrl),
        });
      } catch (emailErr) {
        console.error('[AdminCreateUser] Error sending welcome email:', emailErr.message);
      }
    }

    res.json({ success: true, user: { id, name: finalName, nombre: finalNombre, apellido: finalApellido, email, role: userRole, blocked: 0, casilla } });
  } catch (error) {
    console.error(error);
...
app.get('/api/transacciones', authenticateToken, async (req, res) => {
  try {
    ...
    const totalGastosResult = await db.get("SELECT COALESCE(SUM(monto), 0) as total FROM transacciones_extraidas" + whereClause + " AND (tipo_transaccion IS NULL OR tipo_transaccion = 'gasto') AND categoria NOT IN ('Interno', 'No es Gasto', 'No es Ingreso')", ...filterValues);
    ...
    const result = { transactions, summary, bankTotals: bankTotalsRows, total, pendientes_count, lastCheck: getLastCheckTime(), total_ingresos: totalIngresosResult.total, total_gastos: totalGastosResult.total };
