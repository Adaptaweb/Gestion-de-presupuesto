...
export default function CategoriasConfig({
  ...
  const handleDragStart = (e, cat, idx) => {
    ...
    e.dataTransfer.setData('text/plain', JSON.stringify({ id: cat.id, idx }));
          ...
          <table className="w-full">
            ...
            <tbody>
              {sortedCats.map((cat, idx) => {
                ...
                const showIndicator = dragCat && dragOverIdx === idx && dragCat.id !== cat.id;
                const dragIdx = sortedCats.findIndex(c => c.id === dragCat?.id);
                const insertBefore = showIndicator && (dragIdx === -1 || dragIdx >= idx);
